...
class JobOpportunity < ApplicationRecord
  ...
  validates :title, :company, :description_job, :skills, :salary_range,
            :deadline, :region, :office_functions, presence: true
  ...
  def deadline_cannot_be_in_the_past
    errors.add(:deadline, 'não pode estar no passado') if deadline.present? && deadline < Date.today
  end
end
